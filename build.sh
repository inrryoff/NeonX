...
build_arm64() {
    ...
    compile_target "android-n.c" "$OUTPUT_DIR/neonx_arm64" "aarch64-linux-musl" $PERF_FLAGS $SECURE_FLAGS $MACRO_FLAGS -lm
}

build_arm32() {
    print_info "Compilando: ARMv7 32-bits (Hard Float)..."
    compile_target "android-n.c" "$OUTPUT_DIR/neonx_arm32" "arm-linux-musleabihf" $PERF_FLAGS $SECURE_FLAGS $MACRO_FLAGS -lm
}

build_linux_x64() {
    print_info "Compilando: Linux PC (x86_64 Estático)..."
    compile_target "linux-n.c" "$OUTPUT_DIR/neonx_linux_x64" "x86_64-linux-musl" $PERF_FLAGS $SECURE_FLAGS $MACRO_FLAGS -lm
}

build_linux_x86() {
    print_info "Compilando: Linux PC (x86 Estático)..."
    compile_target "linux-n.c" "$OUTPUT_DIR/neonx_linux_x86" "x86-linux-musl" $PERF_FLAGS $SECURE_FLAGS $MACRO_FLAGS -lm
}
...
if [ "$1" == "--ci" ]; then
    ...
    zig cc linux-n.c -o "$OUTPUT_DIR/neonx_linux_x64" -target x86_64-linux-musl $PERF_FLAGS $SECURE_FLAGS $MACRO_FLAGS -lm

    sign_binary "$OUTPUT_DIR/neonx_linux_x64"
    
    print_success "Ambiente de CI preparado e binário selado!"
    exit 0
fi
# =====================================================
