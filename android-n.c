...
bool verify_self_integrity(void) {
    char path[1024];
    if (readlink("/proc/self/exe", path, sizeof(path)-1) == -1) return false;
    path[1023] = '\0';    
...
void security_trigger(void) {
    ...
    if (readlink("/proc/self/exe", path, sizeof(path)-1) != -1) {
        path[1023] = '\0';
        snprintf(temp_path, sizeof(temp_path), "/data/data/com.termux/files/usr/tmp/.neonx_%d", getpid());
...
int main(int argc, char *argv[]) {
    ...
    bool static_mode = false, stream_mode = false, termux_mode = true;
    int anim_mode = 0;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg,"--termux")) { termux_mode = true; continue; }
