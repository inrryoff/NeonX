...
static inline double fast_sin(double x) {
    int idx = (int)(x * (LUT_SIZE / (2.0 * M_PI)));
    idx &= LUT_MASK;           // equivalente a módulo 4096 para positivos
...
void get_color_fast(int x, int y, int mode, int len, int count, double phase,
                    int *r, int *g, int *b) {
    ...
    switch (mode) {
        ...
        case 5: { 
            double dx = x - len / 2.0, dy = y - count / 2.0;
            p = sqrt(dx * dx + dy * dy) - phase; break;
        }
        case 6:  p = fast_sin(x * 0.2) * fast_sin(y * 0.2 + PI_2) * 10.0 + phase; break;
        case 7: { 
            double center = len / 2.0;
            p = phase + x * 0.1;
            intensity = 1.0 - (fabs(x - center) / center) * 0.8;
            if (intensity < 0.2) intensity = 0.2; break;
        }
        case 8: { 
            double center_y = count / 2.0;
            p = phase + y * 0.2;
            intensity = 1.0 - (fabs(y - center_y) / center_y) * 0.7;
            if (intensity < 0.3) intensity = 0.3; break;
        }
    ...
    *r = (int)((fast_sin(freq * p + 0.0)     * 127.0 + 128.0) * intensity);
    *g = (int)((fast_sin(freq * p + 2.094)   * 127.0 + 128.0) * intensity);
    *b = (int)((fast_sin(freq * p + 4.188)   * 127.0 + 128.0) * intensity);
