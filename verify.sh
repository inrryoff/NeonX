...
for cmd in minisign sha256sum curl unzip awk grep head tail chmod; do
...
if [[ "$BINARY" == *.zip ]]; then
    ...
    if [[ -z "$binary_file" ]]; then
        echo -e "${RED}❌ Nenhum binário executável encontrado dentro do ZIP.${NC}"
        exit 1
    fi
else
    binary_file="$BINARY"
...
echo -e "${YELLOW}🔍 Checando assinatura EdDSA interna (Monocypher/C)...${NC}"
chmod +x "$binary_file"

set +e
if [[ "$REAL_NAME" == *.exe ]]; then
    internal_exit_code=126
else
...
if [[ $internal_exit_code -eq 0 ]]; then
...
elif [[ $internal_exit_code -eq 126 || $internal_exit_code -eq 127 ]]; then
    ...
    COMPATIBILITY_WARNING="\n${YELLOW}   ⚠ AVISO: O Hash e o Minisign confirmam que o binário é\n   100% OFICIAL, mas a assinatura interna não pôde ser\n   testada localmente (Cross-compile ou formato .exe).\n   Teste-o de forma nativa no sistema de destino.${NC}"
else
